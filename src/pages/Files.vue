  ...
  <div>
    ...
    >
      <v-card-title>
        ...
        />
        <v-item-group
          class="v-btn-toggle my-5 my-sm-0 col-12 col-sm-auto px-0 py-0"
          name="controllers"
        >
          <v-btn
            @click="clickUploadButton"
            :title="$t('Files.UploadNewGcode')"
            class="primary flex-grow-1"
          ...
          >
            <template v-slot:activator="{ on, attrs }">
              <v-btn class="flex-grow-1" v-bind="attrs" v-on="on"
                ><v-icon class="">mdi-cog</v-icon></v-btn
              >
            </template>
            <v-list>
          ...
          <v-tooltip top>
            <template v-slot:activator="{ on, attrs }">
              <span v-bind="attrs" v-on="on">{{
                $t("Files.FreeDisk") + formatFilesize(disk_usage.free)
              }}</span>
            </template>
            <span>
      ...
      <div
        class="dragzone"
        :style="
          'visibility: ' + dropzone.visibility + '; opacity: ' + dropzone.hidden
        "
      >
    ...
    >
      <v-list>
        ...
        <v-list-item
          @click="editFile(contextMenu.item)"
          v-if="!contextMenu.item.isDirectory"
        >
    ...
    <v-dialog
      v-model="dialogPrintFile.show"
      :max-width="getThumbnailWidth(dialogPrintFile.item)"
    >
      <v-card>
        <v-img
          contain
        ...
        <v-card-text
          >{{
            $t("Files.DoYouWantToStart") + dialogPrintFile.item.filename
          }}?</v-card-text
        >
        <v-card-actions>
          <v-spacer></v-spacer>
    ...
    >
      ...
      <span v-if="editor.progress.total > 1" class="mr-1"
        >({{ formatFilesize(editor.progress.loaded) }}/{{
          formatFilesize(editor.progress.total)
        }})</span
      >
      {{ Math.round((100 * editor.progress.loaded) / editor.progress.total) }} %
      @ {{ editor.progress.speed }}/s<br />
      <v-progress-linear
        class="mt-2"
        :value="(100 * editor.progress.loaded) / editor.progress.total"
      ></v-progress-linear>
    ...
    >
      ...
      {{ formatFilesize(Math.round(uploadSnackbar.speed)) }}/s<br />
      <v-progress-linear
        class="mt-2"
...
export default {
  ...
  computed: {
    ...
    showHiddenFiles: {
      ...
      set: function (newVal) {
        return this.$store.dispatch("gui/setSettings", {
          gcodefiles: { showHiddenFiles: newVal },
        });
      },
    },
    showPrintedFiles: {
      ...
      set: function (newVal) {
        return this.$store.dispatch("gui/setSettings", {
          gcodefiles: { showPrintedFiles: newVal },
        });
      },
    },
    countPerPage: {
      ...
      set: function (newVal) {
        return this.$store.dispatch("gui/setSettings", {
          gcodefiles: { countPerPage: newVal },
        });
      },
    },
    hideMetadataColums: {
      ...
      set: function (newVal) {
        return this.$store.dispatch("gui/setSettings", {
          gcodefiles: { hideMetadataColums: newVal },
        });
      },
    },
    disk_usage: {
  ...
  methods: {
    ...
    doUploadFile: function (file) {
      ...
      formData.append(
        "file",
        file,
        (this.currentPath + "/" + filename).substring(7)
      );
      ...
      return new Promise((resolve) => {
        ...
        axios
          .post(
            "//" + this.hostname + ":" + this.port + "/server/files/upload",
            formData,
            {
              cancelToken: this.uploadSnackbar.cancelTokenSource.token,
              headers: { "Content-Type": "multipart/form-data" },
              onUploadProgress: (progressEvent) => {
                this.uploadSnackbar.percent =
                  (progressEvent.loaded * 100) / progressEvent.total;
                if (this.uploadSnackbar.lastProgress.time) {
                  const time =
                    progressEvent.timeStamp -
                    this.uploadSnackbar.lastProgress.time;
                  const data =
                    progressEvent.loaded -
                    this.uploadSnackbar.lastProgress.loaded;
      ...
      axios
        .get(url, {
          ...
          onDownloadProgress: (progressEvent) => {
            ...
            const diffTime =
              progressEvent.timeStamp - this.editor.progress.lastTimestamp;
    ...
    async saveFile() {
      let file = new File(
        [this.editor.sourcecode],
        encodeURI(this.editor.item.filename)
      );
    ...
    renameDirectoryAction() {
      ...
      this.$socket.sendObj(
        ...
        {
          source:
            this.currentPath + "/" + this.dialogRenameDirectory.item.filename,
    ...
    createDirectoryAction: function () {
      if (
        this.dialogCreateDirectory.name.length &&
        this.dialogCreateDirectory.name.indexOf(" ") === -1
      ) {
    ...
    clickRowGoBack: function () {
      this.currentPath = this.currentPath.substr(
        0,
    ...
    loadPath: function () {
      ...
      if (!this.showHiddenFiles) {
        this.files = this.files.filter(
          (file) =>
            file.filename !== "thumbs" && file.filename.substr(0, 1) !== "."
        );
      }
    ...
    sortFiles(items, sortBy, sortDesc) {
      ...
      if (items !== undefined) {
        ...
        items.sort(function (a, b) {
          ...
          if (
            a[sortBy].constructor === String &&
            b[sortBy].constructor === String
          ) {
          ...
          if (a[sortBy] instanceof Array && b[sortBy] instanceof Array) {
            const reducedA = a[sortBy].length
              ? a.filament.reduce((a, b) => a + b)
              : 0;
            const reducedB = b[sortBy].length
              ? b.filament.reduce((a, b) => a + b)
              : 0;
    ...
    refreshMetadata: function (data) {
      ...
      for (let i = data.pageStart; i < data.pageStop; i++) {
        if (items[i] && !items[i].isDirectory && !items[i].metadataPulled) {
          let filename = (this.currentPath + "/" + items[i].filename).substring(
            7
          );
          this.$socket.sendObj(
  ...
  watch: {
    "editor.init"(val) {
      if (val) {
        setTimeout(() => {
          this.editor.monaco = monaco.editor.create(
      ...
      handler(newVal) {
        if (newVal !== undefined && typeof newVal === "object") {
          Object.entries(newVal).forEach((value) => {
            if (
              this.headers.filter((header) => header.value === value[0]).length
            ) {
              this.headers.filter(
                (header) => header.value === value[0]
              )[0].visible = value[1];
            }
          });
        }
    ...
    hideMetadataColums: function (newVal) {
      newVal.forEach((key) => {
        let headerElement = this.headers.find(
