      ...
      <v-col :class="pwm ? 'pb-1' : 'pb-3'">
        <v-subheader class="_fan-slider-subheader">
          <v-icon
            small
            :class="
              'mr-2 ' + (value >= off_below && value > 0 ? 'icon-rotate' : '')
            "
          ...
          <small
            v-if="rpm || rpm === 0"
            :class="'mr-3 ' + (rpm === 0 && value > 0 ? 'red--text' : '')"
            >{{ Math.round(rpm) }} RPM</small
          >
          <span
            class="font-weight-bold"
            v-if="!controllable || (controllable && pwm)"
            >{{ Math.round(parseFloat(value) * 100) }} %</span
          >
  ...
  methods: {
    ...
    sendCmd() {
      if (this.sliding) {
        ...
        if (this.type === "output_pin")
          gcode = "SET_PIN PIN=" + this.name + " VALUE=" + l_value.toFixed(2);
    ...
    switchOutputPin() {
      ...
      const gcode =
        ...
        this.name +
        " VALUE=" +
        (this.value * this.multi).toFixed(2);
      this.$store.commit("server/addEvent", {
        message: gcode,
        type: "command",
      });
