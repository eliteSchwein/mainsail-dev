  ...
  <v-app :class="getTheme">
    ...
    >
      <div id="nav-header">
        <img :src="sidebarLogo" />
        <v-toolbar-title>{{
          printername !== "" ? printername : hostname
        }}</v-toolbar-title>
      </div>
      <ul :class="'navi ' + getTheme" :expand="$vuetify.breakpoint.mdAndUp">
        <printer-selecter></printer-selecter>
        <li
          v-for="(category, index) in routes"
          :key="index"
          :prepend-icon="category.icon"
          :class="[
            category.path !== '/' && currentPage.includes(category.path)
              ? 'active'
              : '',
            'nav-item',
          ]"
        ...
        >
          ...
          >
            ...
            <v-icon
              small
              color="warning"
              style="top: 11px; left: 32px; position: absolute"
          ...
          <ul class="child">
            <li
              ...
              class="nav-item"
              v-bind:key="`${index}-${pageIndex}`"
            >
              <router-link
                :to="page.path"
                class="nav-link"
                @click.prevent
                v-if="klippy_state !== 'error' || page.alwaysShow"
              >
      ...
      <p
        id="sidebarVersions"
        class="mb-0 text-body-2 pl-3 pb-2"
        v-if="showVersion"
      >
        v{{ getVersion
        }}<span class="" v-if="klipperVersion">
          -
          {{ klipperVersion.substr(0, klipperVersion.lastIndexOf("-")) }}</span
        >
      </p>
    </v-navigation-drawer>
    ...
    <v-app-bar app elevate-on-scroll>
      ...
      <template v-if="debugMode" class="mr-3">
        <span v-if="'jsHeapSizeLimit' in debugModeData.memory" class="mr-5"
          >L: {{ formatFilesize(debugModeData.memory.jsHeapSizeLimit) }}</span
        >
        <span v-if="'totalJSHeapSize' in debugModeData.memory" class="mr-5"
          >T: {{ formatFilesize(debugModeData.memory.totalJSHeapSize) }}</span
        >
        <span v-if="'usedJSHeapSize' in debugModeData.memory" class="mr-5"
          >U: {{ formatFilesize(debugModeData.memory.usedJSHeapSize) }}</span
        >
      </template>
      <input
        type="file"
      ...
      <v-btn
        ...
        ><v-icon class="mr-sm-2">mdi-alert-circle-outline</v-icon
    ...
    >
      <v-scroll-y-transition>
        <v-container
          fluid
          id="page-container"
          class="container px-3 px-sm-6 py-sm-6 mx-auto"
        >
          <keep-alive>
    ...
    <v-footer
      ...
      class="d-block"
      style="z-index: 20000"
      v-if="keyboardActivated && keyboardVisible"
    >
      ...
      <vue-touch-keyboard
        :class="getTheme"
        @click.native="clickKeyboard"
        style="z-index: 200"
        :options="keyboardOptions"
        :layout="keyboardLayout"
        :cancel="hideKeyboard"
        :accept="clickAcceptKeyboard"
        :input="keyboardInput"
        :next="clearKeyboard"
      />
    </v-footer>
    ...
    >
      <strong>{{ $t("App.Uploading") }} {{ uploadSnackbar.filename }}</strong
      ><br />
...
export default {
  ...
  computed: {
    ...
    ...mapState({
      ...
      save_config_pending: (state) =>
        state.printer.configfile.save_config_pending,
    ...
    ...mapGetters(["getTitle", "getVersion"]),
    print_percent: {
      get() {
  ...
  mounted() {
    bus.$on("showkeyboard", (event) => {
      ...
      if (
        this.keyboardInput.dataset.layout == "normal" &&
        this.virtualKeyboardCompact()
      ) {
  ...
  methods: {
    ...
    clearKeyboard() {
      (this.keyboardInput.value = null), (this.keyboardInputvalue = null);
    },
    clickAcceptKeyboard() {
      this.hideKeyboard();
    },
    ...
    doUploadAndStart(file) {
      ...
      return new Promise((resolve) => {
        ...
        axios
          .post(
            "//" + this.apiHost + ":" + this.apiPort + "/server/files/upload",
            formData,
            {
          ...
          .catch(() => {
            this.uploadSnackbar.status = false;
            this.$store.commit("socket/removeLoading", {
    ...
    getMemoryUsage() {
      if ("memory" in performance) {
        if ("jsHeapSizeLimit" in performance.memory)
          this.debugModeData.memory.jsHeapSizeLimit =
            performance.memory.jsHeapSizeLimit;

        if ("totalJSHeapSize" in performance.memory)
          this.debugModeData.memory.totalJSHeapSize =
            performance.memory.totalJSHeapSize;

        if ("usedJSHeapSize" in performance.memory)
          this.debugModeData.memory.usedJSHeapSize =
