  ...
  >
    ...
    <v-list dense>
      ...
      <v-list-item
        ...
        v-if="boolWebcam"
        link
        @click="doServiceRestartWebcam()"
      >
        <v-list-item-title
          ><v-icon class="mr-2" small>mdi-restart</v-icon
          >Webcam</v-list-item-title
        >
      </v-list-item>
      <div v-if="countPowerDevices">
        ...
        >
          <v-list-item-title>
            <v-icon
              class="mr-2"
              :color="device.status === 'on' ? '' : 'grey darken-2'"
              >mdi-{{
                device.status === "on" ? "toggle-switch" : "toggle-switch-off"
              }}</v-icon
            >{{ device.device }}
          </v-list-item-title>
        </v-list-item>
      ...
      <v-list-item class="minheight30" link @click="doHostReboot()">
        <v-list-item-title
          ...
          >{{ $t("App.Reboot") }}</v-list-item-title
        >
      </v-list-item>
      <v-list-item class="minheight30" link @click="doHostShutdown()">
        <v-list-item-title
          ><v-icon class="mr-2" small>mdi-power</v-icon
          >{{ $t("App.Shutdown") }}</v-list-item-title
        >
      </v-list-item>
    </v-list>
  </v-menu>
...
export default {
  ...
  computed: {
    ...
    boolWebcam: {
      get() {
        return (
          this.$store.state.gui.dashboard.boolWebcam ||
          this.$store.state.gui.webcam.bool
        );
      },
    },
  },
  methods: {
    ...
    setPower(device, value) {
      let rpc =
        value === 1 ? "machine.device_power.on" : "machine.device_power.off";
      Vue.prototype.$socket.sendObj(
        rpc,
        { [device.device]: null },
        "server/power/responseToggle"
      );
    },
    doReload: function () {
      this.showMenu = false;
    ...
    doFirmwareRestart: function () {
      ...
      this.$socket.sendObj("printer.gcode.script", {
    ...
    doServiceRestartMoonraker: function () {
      this.showMenu = false;
      this.$socket.sendObj("machine.services.restart", {
        service: "moonraker",
      });
    },
