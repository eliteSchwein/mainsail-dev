...
export default {
          ...
          formatter: (datasets) => {
            ...
            datasets.forEach((dataset) => {
              if (
                ...
                const mainDataset = this.series.find(
                  (tmpDataset) => tmpDataset.name === dataset.seriesName
                );
                if (mainDataset)
                  output +=
                    '<span style="' +
                ...
                const target = datasets.find(
                  (tmpDataset) =>
                    tmpDataset.seriesName === dataset.seriesName + "_target"
                );
                if (target) output += " / " + target.value[1].toFixed(1);
                output += "°C";
                const power = datasets.find(
                  (tmpDataset) =>
                    tmpDataset.seriesName === dataset.seriesName + "_power"
                );
                if (power) output += " [ " + power.value[1].toFixed(0) + "% ]";
                const speed = datasets.find(
                  (tmpDataset) =>
                    tmpDataset.seriesName === dataset.seriesName + "_speed"
                );
  ...
  computed: {
    ...
    maxHistory: {
      get() {
        return (
          this.$store.getters["server/getConfig"](
            "server",
  ...
  methods: {
    createChart() {
      if (document.getElementById("tempchart") && this.chart === null) {
        this.chart = echarts.init(document.getElementById("tempchart"), null, {
          renderer: "canvas",
        });
