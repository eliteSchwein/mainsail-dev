      ...
      <v-toolbar flat dense>
        <v-toolbar-title>
          <span class="subheading"
            ...
            >{{ $t("Settings.GeneralPanel.General") }}</span
          >
        </v-toolbar-title>
      </v-toolbar>
      <v-card-text>
            ...
            <v-switch v-model="displayCancelPrint" hide-details class="mt-0">
              <template v-slot:label>
                ...
                <v-tooltip right>
                  ...
                  <span>{{
                    $t("Settings.GeneralPanel.ShowCANCEL_PRINT")
                  }}</span>
                </v-tooltip>
              </template>
            </v-switch>
        ...
        <v-row>
          <v-col class="text-center">
            <v-btn @click="dialogResetMainsail = true">{{
              $t("Settings.GeneralPanel.FactoryReset")
            }}</v-btn>
          </v-col>
        </v-row>
      </v-card-text>
    ...
    <v-dialog v-model="dialogResetMainsail" persistent :width="300">
      <v-card dark>
        <v-toolbar flat dense color="primary">
          <v-toolbar-title>
            <span class="subheading">
              <v-icon class="mdi mdi-help-circle" left></v-icon>
              {{ $t("Settings.GeneralPanel.FactoryReset") }}
            </span>
          </v-toolbar-title>
          <v-spacer></v-spacer>
          <v-btn small class="minwidth-0" @click="dialogResetMainsail = false"
        ...
        <v-card-text class="pt-3">
          <v-container class="pb-0">
            <v-row>
              <v-col>
                <p class="text-center mb-0">
                  {{ $t("Settings.GeneralPanel.FactoryInfo") }}
                </p>
              </v-col>
            </v-row>
            ...
            <v-row>
              <v-col class="text-center">
                <v-btn color="red" @click="resetMainsail">
  ...
  computed: {
    ...
    displayCancelPrint: {
      ...
      set(displayCancelPrint) {
        return this.$store.dispatch("gui/setSettings", {
          general: { displayCancelPrint: displayCancelPrint },
        });
      },
    },
    displayZOffsetStandby: {
      ...
      set(displayZOffsetStandby) {
        return this.$store.dispatch("gui/setSettings", {
          general: { displayZOffsetStandby: displayZOffsetStandby },
        });
      },
    },
  },
