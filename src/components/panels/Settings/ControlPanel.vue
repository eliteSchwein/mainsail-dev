      ...
      <v-toolbar flat dense>
        <v-toolbar-title>
          <span class="subheading"
            ...
            >{{ $t("Settings.ControlPanel.Control") }}</span
          >
        </v-toolbar-title>
      </v-toolbar>
          ...
          <v-row>
            ...
            <v-col col-6>
              <v-text-field
                ...
                hide-details="auto"
                ref="feedrateZ"
                :rules="[(v) => v > 1 || 'Minimum speed is 1']"
              ></v-text-field>
            </v-col>
          </v-row>
          <v-row>
            <v-col class="py-0">
              <v-switch
                v-model="useCross"
                :label="$t('Settings.ControlPanel.EnableCross')"
                hide-details="auto"
                class="mt-0"
              ></v-switch>
            </v-col>
          ...
          <template v-if="useCross">
              ...
              <v-col>
                <v-combobox
                  ...
                  :rules="[
                    ...
                    (v) =>
                      v.length < 9 ||
                      'For narrow screens it is recommended to enter max. 3 values.',
                  ]"
                ></v-combobox>
              </v-col>
            </v-row>
            <v-row>
              <v-col class="pt-0">
                <v-switch
                  v-model="reverseX"
                  :label="$t('Settings.ControlPanel.ReverseXMovement')"
                  hide-details="auto"
                  class="mt-0"
                ></v-switch>
                <v-switch
                  v-model="reverseY"
                  :label="$t('Settings.ControlPanel.ReverseYMovement')"
                  hide-details="auto"
                  class="mt-0"
                ></v-switch>
                <v-switch
                  v-model="reverseZ"
                  :label="$t('Settings.ControlPanel.ReverseZMovement')"
                  hide-details="auto"
                  class="mt-0"
                ></v-switch>
              </v-col>
    ...
    <v-card class="mt-6">
      <v-toolbar flat dense>
        <v-toolbar-title>
          <span class="subheading"
            ><v-icon left>mdi-tune</v-icon
            >{{ $t("Settings.ControlPanel.Extruder") }}</span
          >
        </v-toolbar-title>
      </v-toolbar>
      <v-card-text class="pa-0">
            ...
            <v-col>
              <v-combobox
                ...
                :rules="[
                  ...
                  (v) =>
                    v.length < 6 ||
                    'For narrow screens it is recommended to enter max. 5 values.',
                ]"
              ></v-combobox>
            </v-col>
          </v-row>
...
export default {
  ...
  computed: {
    ...
    feedrateXY: {
      ...
      set(feedrate) {
        return this.$store.dispatch("gui/setSettings", {
          dashboard: { control: { feedrateXY: feedrate } },
        });
      },
    },
    stepsAll: {
      ...
      set(steps) {
        ...
        return this.$store.dispatch("gui/setSettings", {
          dashboard: { control: { stepsAll: absSteps } },
        });
      },
    },
    stepsXY: {
      ...
      set(steps) {
        ...
        for (const value of steps) absSteps.push(Math.abs(value));
        return this.$store.dispatch("gui/setSettings", {
          dashboard: { control: { stepsXY: absSteps } },
        });
      },
    },
    feedrateZ: {
      ...
      set(value) {
        return this.$store.dispatch("gui/setSettings", {
          dashboard: { control: { feedrateZ: value } },
        });
      },
    },
    stepsZ: {
      ...
      set(steps) {
        ...
        for (const value of steps) absSteps.push(Math.abs(value));
        return this.$store.dispatch("gui/setSettings", {
          dashboard: { control: { stepsZ: absSteps } },
        });
      },
    },
    feedamountsE: {
      ...
      set(amounts) {
        ...
        return this.$store.dispatch("gui/setSettings", {
          dashboard: { extruder: { feedamounts: absAmounts } },
        });
      },
    },
    feedratesE: {
      ...
      set(rates) {
        ...
        return this.$store.dispatch("gui/setSettings", {
          dashboard: { extruder: { feedrates: absRates } },
        });
      },
    },
  },
