  ...
  <div>
    <v-card>
      <v-toolbar flat dense>
        <v-toolbar-title>
          <span class="subheading"
            ...
            >{{ $t("Settings.PresetsPanel.PreheatPresets") }}</span
          >
        </v-toolbar-title>
      </v-toolbar>
      <v-card-text class="py-3">
        <v-container>
          <v-row
            v-for="(preset, index) in this['gui/getPreheatPresets']"
            v-bind:key="index"
          >
            <v-col
            ...
            >
              <v-row align="center">
                ...
                <v-col class="col-6">
                  <div
                    v-for="[key, value] in Object.entries(preset.values)"
                    v-bind:key="key"
                  >
                    <span class="text-no-wrap" v-if="value.bool"
                      >{{ convertPresetName(key, value) }}:
                      {{ value.value }}°C</span
                    >
                  </div>
                </v-col>
                <v-col class="col-auto text-right"
                  ><v-btn
                    ...
                    v-on:click.stop.prevent="editPreset(preset)"
          ...
          <v-row>
            <v-col
              class="rounded transition-swing secondary py-2 px-2 mb-2"
              style="cursor: pointer"
            >
              <v-row align="center">
                <v-col class="pl-6">
                ...
                <v-col class="col-auto text-right"
                  ><v-btn
                    small
                    class="minwidth-0 float-right"
                    v-on:click.stop.prevent="editCooldown"
                    ><v-icon small>mdi-pencil</v-icon></v-btn
                  ></v-col
                >
              </v-row>
          ...
          <v-row>
            <v-col class="text-center mt-0">
              <v-btn @click="createPreset">{{
                $t("Settings.PresetsPanel.AddPreset")
              }}</v-btn>
            </v-col>
          </v-row>
        </v-container>
      ...
      <v-card dark>
        <v-toolbar flat dense color="primary">
          <v-toolbar-title>
            <span class="subheading">
              ...
              {{
                dialog.index === null
                  ? $t("Settings.PresetsPanel.Create")
                  : $t("Settings.PresetsPanel.Edit")
              }}
              {{ $t("Settings.PresetsPanel.Preset") }}
            </span>
          </v-toolbar-title>
          <v-spacer></v-spacer>
        ...
        <v-card-text class="pt-3">
          <v-container class="pb-0">
            <v-form v-model="dialog.valid" @submit.prevent="savePreset">
                ...
                <v-row>
                  <v-col class="col-12 col-sm-6">
                    ...
                    <v-row
                      ...
                      v-for="heater of this['printer/getHeaters']"
                      v-bind:key="heater.name"
                      align="center"
                    >
                      <v-checkbox
                    ...
                    <v-row
                      class="mt-2 mx-0 mb-2"
                      v-for="fan of this['printer/getTemperatureFans']"
                      v-bind:key="'temperature_fan ' + fan.name"
                      align="center"
                    >
                      <v-checkbox
                        v-model="
                          dialog.values['temperature_fan ' + fan.name].bool
                        "
                        hide-details
                        class="shrink mt-0"
                      ></v-checkbox>
                      <v-text-field
                        v-model="
                          dialog.values['temperature_fan ' + fan.name].value
                        "
                ...
                <v-row class="mt-3" v-if="dialog.boolInvalidMin">
                  <v-col class="py-0">
                    <v-alert dense text type="error">{{
                      $t("Settings.PresetsPanel.PresetInfo")
                    }}</v-alert>
                  </v-col>
                </v-row>
                <v-row class="mt-3">
                  <v-col class="text-center">
                    ...
                    >
                      {{
                        dialog.index === null
                          ? $t("Settings.PresetsPanel.Store")
                          : $t("Settings.PresetsPanel.Update")
                      }}
                      {{ $t("Settings.PresetsPanel.Preset") }}
                    </v-btn>
                  </v-col>
      ...
      <v-card dark>
        <v-toolbar flat dense color="primary">
          <v-toolbar-title>
            <span class="subheading">
              <v-icon class="mdi mdi-fire" left></v-icon>
              {{ $t("Settings.PresetsPanel.EditCooldown") }}
            </span>
          </v-toolbar-title>
          <v-spacer></v-spacer>
          <v-btn small class="minwidth-0" @click="cooldownDialog.bool = false"
            ><v-icon small>mdi-close-thick</v-icon></v-btn
          >
        </v-toolbar>
        <v-card-text class="pt-3">
          <v-container class="pb-0">
            ...
            >
              ...
              <v-row>
                <v-col class="text-center">
                  <v-btn color="white" outlined type="submit">
...
export default {
  ...
  data: function () {
    return {
      ...
      rules: {
        ...
        unique: (value) =>
          !this.existsPresetName(value) || "Name already exists",
      },
      cooldownDialog: {
        bool: false,
        gcode: "",
  ...
  methods: {
    ...
    convertPresetName(name, value) {
      if (value.type === "temperature_fan")
        name = name.replace("temperature_fan ", "");

      return this.convertName(name);
    },
    existsPresetName(name) {
      return (
        this["gui/getPreheatPresets"].findIndex(
          (preset) => preset.name === name && preset.index !== this.dialog.index
        ) >= 0
      );
    },
    createPreset() {
    ...
    savePreset() {
      ...
      if (setValues === 0) this.dialog.boolInvalidMin = true;
        ...
        for (const key of Object.keys(this.dialog.values)) {
          this.dialog.values[key].value = parseInt(
            this.dialog.values[key].value
          );
        }
    ...
    saveCooldown() {
      if (this.cooldownDialog.valid) {
        this.$store.dispatch("gui/setSettings", {
          cooldown_gcode: this.cooldownDialog.gcode,
        });
