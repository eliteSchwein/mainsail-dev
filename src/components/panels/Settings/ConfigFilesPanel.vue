        ...
        <v-toolbar dark color="primary">
          ...
          <v-toolbar-items>
            ...
            <v-btn
              ...
              ><v-icon small class="mr-1">mdi-help</v-icon>Config
              Reference</v-btn
            >
            <v-divider
              white
              vertical
              v-if="currentPath !== '/config_examples'"
              class="d-none d-md-flex"
            ></v-divider>
            <v-btn
              dark
              ...
              ><span class="d-none d-sm-inline">{{
                $t("Settings.ConfigFilesPanel.SaveClose")
              }}</span></v-btn
            >
            <v-divider
              white
              vertical
              v-if="
                currentPath !== '/config_examples' &&
                !['printing', 'paused'].includes(printer_state)
              "
            ...
            <v-btn
              ...
              >{{ $t("Settings.ConfigFilesPanel.SaveRestart") }}</v-btn
            >
          </v-toolbar-items>
        </v-toolbar>
        <div
    ...
    <v-col>
      <v-card>
        <v-card-title>
          ...
          >
            <v-btn
              v-if="currentPath !== '' && currentPath !== '/config_examples'"
              class="flex-grow-1"
            ...
            >
            <v-btn class="flex-grow-1" @click="refreshFileList"
              ><v-icon>mdi-refresh</v-icon></v-btn
            >
            <v-menu
              :offset-y="true"
              :title="$t('Settings.ConfigFilesPanel.SetupCurrentList')"
            >
              ...
              <v-list>
                <v-list-item class="minHeight36">
                  <v-checkbox
                    ...
                    :label="$t('Settings.ConfigFilesPanel.HiddenFiles')"
                  ></v-checkbox>
                </v-list-item>
              </v-list>
            </v-menu>
        ...
        <v-card-subtitle
          >{{ $t("Settings.ConfigFilesPanel.CurrentPath") }}:
          {{
        ...
        >
          <template #no-data>
            <div class="text-center">
              {{ $t("Settings.ConfigFilesPanel.Empty") }}
            </div>
          </template>
      ...
      >
        <v-list>
          ...
          >
            ...
            {{ $t("Settings.ConfigFilesPanel.EditFile") }}
          </v-list-item>
          <v-list-item
            @click="downloadFile"
            v-if="!contextMenu.item.isDirectory"
          >
            ...
            {{ $t("Settings.ConfigFilesPanel.Download") }}
          </v-list-item>
          <v-list-item
          ...
          >
            ...
            {{ $t("Settings.ConfigFilesPanel.Rename") }}
          </v-list-item>
          <v-list-item
            @click="removeFile"
            v-if="
          ...
          >
            ...
            {{ $t("Settings.ConfigFilesPanel.Delete") }}
          </v-list-item>
          <v-list-item
      ...
      >
        <v-card style="position: relative">
          ...
          <div
            ...
            style="max-height: calc(100vh - 64px); overflow: auto"
          >
            <img
              v-if="image.url"
              :src="image.url"
      ...
      <v-dialog v-model="dialogRenameFile.show" max-width="400">
        <v-card>
          <v-card-title class="headline">{{
            $t("Settings.ConfigFilesPanel.RenameFile")
          }}</v-card-title>
          <v-card-text>
            <v-text-field
              ...
              @blur="hideKeyboard"
              data-layout="normal"
              :label="$t('Settings.ConfigFilesPanel.Name')"
              required
          ...
          <v-card-actions>
            ...
            <v-btn color="" text @click="dialogRenameFile.show = false">{{
              $t("Settings.ConfigFilesPanel.Cancel")
            }}</v-btn>
      ...
      <v-dialog v-model="dialogCreateFile.show" max-width="400">
        <v-card>
          <v-card-title class="headline">{{
            $t("Settings.ConfigFilesPanel.CreateFile")
          }}</v-card-title>
          <v-card-text>
            <v-text-field
              @click.native="showKeyboard"
          ...
          <v-card-actions>
            ...
            <v-btn color="primary" text @click="createFileAction">{{
              $t("Settings.ConfigFilesPanel.Create")
            }}</v-btn>
          </v-card-actions>
        </v-card>
      </v-dialog>
      <v-dialog v-model="dialogCreateFolder.show" max-width="400">
        <v-card>
          <v-card-title class="headline">{{
            $t("Settings.ConfigFilesPanel.CreateFolder")
          }}</v-card-title>
          <v-card-text>
            <v-text-field
              @click.native="showKeyboard"
              ...
              :label="$t('Settings.ConfigFilesPanel.Name')"
              required
              v-model="dialogCreateFolder.name"
            ></v-text-field>
          </v-card-text>
          <v-card-actions>
            <v-spacer></v-spacer>
            <v-btn color="" text @click="dialogCreateFolder.show = false">{{
              $t("Settings.ConfigFilesPanel.Cancel")
            }}</v-btn>
            <v-btn color="primary" text @click="createFolderAction">{{
              $t("Settings.ConfigFilesPanel.Create")
            }}</v-btn>
          </v-card-actions>
        </v-card>
      ...
      >
        ...
        ><strong
          >{{ $t("Settings.ConfigFilesPanel.Uploading") }}
          {{ uploadSnackbar.filename }}</strong
        ><br />
