      ...
      <v-toolbar flat dense>
        <v-toolbar-title>
          <span class="subheading">
            ...
            >{{ $t("Settings.WebcamPanel.Webcams") }}
          </span>
        </v-toolbar-title>
      </v-toolbar>
        ...
        <v-container>
          ...
          <v-row>
            <v-col class="text-center mt-0">
              <v-btn @click="createWebcam">{{
                $t("Settings.WebcamPanel.AddWebcam")
              }}</v-btn>
            </v-col>
          </v-row>
        </v-container>
        ...
        <v-toolbar flat dense color="primary">
          <v-toolbar-title>
            <span class="subheading">
              ...
              {{
                dialog.index === null
                  ? $t("Settings.WebcamPanel.CreateWebcam")
                  : $t("Settings.WebcamPanel.EditWebcam")
              }}
            </span>
          </v-toolbar-title>
          <v-spacer></v-spacer>
              ...
              <template v-if="dialog.bool">
                <v-row>
                        ...
                        <v-item-group>
                          <v-menu :offset-y="true" title="Icon">
                            ...
                            <v-list dense class="py-0">
                              <v-list-item
                                v-for="icon of this.iconItems"
                                v-bind:key="icon.value"
                                link
                                @click="setCurrentIcon(icon.value)"
                              >
                  ...
                  >
                    ...
                    <video
                      v-if="['ipstream'].includes(dialog.service)"
                      :src="dialog.url"
                      autoplay
                      :style="webcamStyle"
                    />
                ...
                <v-row class="mt-3">
                  <v-col class="text-center">
                    ...
                    >
                      {{
                        dialog.index === null
                          ? $t("Settings.WebcamPanel.SaveWebcam")
                          : $t("Settings.WebcamPanel.UpdateWebcam")
                      }}
                    </v-btn>
                  </v-col>
                </v-row>
...
export default {
  ...
  data: function () {
    return {
      ...
      rules: {
        required: (value) =>
          value !== "" || this.$t("Settings.WebcamPanel.Required"),
        unique: (value) =>
          !this.existsWebcamName(value) ||
          this.$t("Settings.WebcamPanel.NameAlreadyExists"),
      },
      iconItems: [
        {
          value: "mdi-printer-3d",
          text: this.$t("Settings.WebcamPanel.IconPrinter"),
        },
        {
          value: "mdi-printer-3d-nozzle",
          text: this.$t("Settings.WebcamPanel.IconNozzle"),
        },
        {
          value: "mdi-radiator-disabled",
          text: this.$t("Settings.WebcamPanel.IconBed"),
        },
        { value: "mdi-webcam", text: this.$t("Settings.WebcamPanel.IconCam") },
        {
          value: "mdi-album",
          text: this.$t("Settings.WebcamPanel.IconFilament"),
        },
        { value: "mdi-door", text: this.$t("Settings.WebcamPanel.IconDoor") },
        {
          value: "mdi-raspberry-pi",
          text: this.$t("Settings.WebcamPanel.IconMcu"),
        },
        {
          value: "mdi-campfire",
          text: this.$t("Settings.WebcamPanel.IconHot"),
        },
      ],
      serviceItems: [
        {
          value: "mjpegstreamer",
          text: this.$t("Settings.WebcamPanel.Mjpegstreamer"),
        },
        {
          value: "mjpegstreamer-adaptive",
          text: this.$t("Settings.WebcamPanel.MjpegstreamerAdaptive"),
        },
