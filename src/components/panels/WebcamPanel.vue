        ...
        <v-menu :offset-y="true" title="Webcam">
          <template v-slot:activator="{ on, attrs }">
            <v-btn
              ...
              v-on="on"
            >
              <v-icon small v-if="'icon' in currentCam" class="mr-2">{{
                currentCam.icon
              }}</v-icon>
          ...
          <v-list dense class="py-0">
            <v-list-item link @click="currentCamName = 'all'">
              ...
              <v-list-item-content>
                <v-list-item-title>{{
                  $t("Panels.WebcamPanel.All")
                }}</v-list-item-title>
              </v-list-item-content>
            </v-list-item>
            <v-list-item
              v-for="webcam of this.webcams"
    ...
    <v-card-text class="px-0 py-0 content d-inline-block">
      <v-row>
        <v-col class="pb-0" style="position: relative">
          <template
            v-if="
              'service' in this.currentCam && this.currentCam.service === 'grid'
            "
          >
          ...
          <template
            v-else-if="
              'service' in this.currentCam &&
              this.currentCam.service === 'mjpegstreamer'
            "
          >
            <webcam-mjpegstreamer
              :cam-settings="this.currentCam"
            ></webcam-mjpegstreamer>
          </template>
          <template
            v-else-if="
              'service' in this.currentCam &&
              this.currentCam.service === 'mjpegstreamer-adaptive'
            "
          >
          ...
          <template
            v-else-if="
              'service' in this.currentCam &&
              this.currentCam.service === 'ipstream'
            "
          >
            <webcam-ipstreamer
              :cam-settings="this.currentCam"
  ...
  computed: {
    currentCamName: {
      get() {
        ...
        if (
          currentCamName !== undefined &&
          this.webcams.findIndex((webcam) => webcam.name === currentCamName) !==
            -1
        )
        ...
        if (
          currentCamName !== undefined &&
          Array.isArray(this.webcams) &&
          this.webcams.length === 1
        )
    ...
    currentCam() {
      if (this.currentCamName === "all") {
        ...
        const currentCam = this.webcams.find(
          (webcam) => webcam.name === this.currentCamName
        );
