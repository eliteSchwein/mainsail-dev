    ...
    <v-toolbar flat dense>
      <v-toolbar-title>
        <span class="subheading"
          ...
          >{{ $t("Panels.MiniconsolePanel.Console") }}</span
        >
      </v-toolbar-title>
      <v-spacer></v-spacer>
      <v-menu
      ...
      >
        ...
        <v-list>
          ...
          <v-list-item class="minHeight36">
            <v-checkbox
              class="mt-0"
              v-model="boolCustomFilters"
              hide-details
              :label="$t('Panels.MiniconsolePanel.CustomFilters')"
            ></v-checkbox>
          </v-list-item>
        </v-list>
      </v-menu>
      ...
      <v-container class="py-0 px-0">
        <v-row>
          ...
          <v-col class="col-auto align-content-center">
            <v-btn
              color="info"
              class="gcode-command-btn"
              @click="doSend"
              :loading="loadings.includes('sendGcode')"
              :disabled="loadings.includes('sendGcode')"
            >
              <v-icon class="mr-2">mdi-send</v-icon>
              {{ $t("Panels.MiniconsolePanel.Send") }}
            </v-btn>
          </v-col>
        </v-row>
      </v-container>
      ...
      >
        ...
        <template #item="{ item }">
          <tr>
            ...
            <td
              class="log-cell content-cell pl-0 py-2"
              colspan="2"
              style="width: 100%"
            >
              <span
                v-if="item.message"
                :class="'message ' + colorConsoleMessage(item)"
                v-html="formatConsoleMessage(item.message)"
              ></span>
            </td>
          </tr>
        </template>
    ...
    hideWaitTemperatures: {
      ...
      set: function (newVal) {
        return this.$store.dispatch("gui/setSettings", {
          console: { hideWaitTemperatures: newVal },
        });
      },
    },
    boolCustomFilters: {
      ...
      set: function (newVal) {
        return this.$store.dispatch("gui/setSettings", {
          console: { boolCustomFilters: newVal },
        });
      },
    },
  },
  methods: {
    ...
    getAutocomplete(e) {
      ...
      if (this.gcode.length) {
        let commands = this.helplist.filter(
          (element) =>
            element.commandLow.indexOf(this.gcode.toLowerCase()) === 0
        );
        if (commands && commands.length === 1) this.gcode = commands[0].command;
        else {
          let commands = this.helplist.filter((element) =>
            element.commandLow.includes(this.gcode.toLowerCase())
          );
